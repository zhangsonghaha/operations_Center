...
public class SysDbLogController extends BaseController
{
     ...
     * 删除数据库操作日志
     */
    @PreAuthorize("@ss.hasPermi('system:db:log:remove')")
    @Log(title = "数据库操作日志", businessType = BusinessType.DELETE)
    @PostMapping("/delete")
    public AjaxResult remove(@RequestBody Long[] logIds)
    {
        return toAjax(sysDbLogService.deleteSysDbLogByLogIds(logIds));
    }
}
