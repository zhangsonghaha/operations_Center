...
public class OpsReleaseApplyServiceImpl implements IOpsReleaseApplyService {
    ...
    public List<OpsReleaseApply> selectPendingApprovals(OpsReleaseApply query) {
        ...
        return opsReleaseApplyMapper.selectReleasePendingList(userId, userName, getRoleIds(userId), query);
    }
    ...
    public void processApproval(Long id, String status, String comment) {
        ...
        if (apply.getProcessInstanceId() != null) {
            ...
            List<Task> userTasks = workflowService.getTasksForUser(currentUserIdStr, currentUserName, groupIds);
    ...
    public void cancelReleaseApply(Long id) {
        ...
        if (!currentUserId.equals(apply.getApplyUserId())) {
            throw new ServiceException("只有发起人可以撤销申请");
        }

        // Check status: Only Pending (1) can be cancelled? Or Draft (0) is already editable.
        // Usually, once approved (2) or rejected (3), it's done. 
        // So only "1" (Pending) can be cancelled.
        ...
        if (StringUtils.isNotEmpty(apply.getProcessInstanceId())) {
            try {
                org.flowable.engine.RuntimeService runtimeService = com.ruoyi.common.utils.spring.SpringUtils.getBean(org.flowable.engine.RuntimeService.class);
                runtimeService.deleteProcessInstance(apply.getProcessInstanceId(), "Applicant Cancelled");
            } catch (Exception e) {
                // Ignore if process already ended or not found
            }
        }

        // Update status to Draft (0) or Cancelled (custom status, e.g. 6)?
        // Usually "Withdraw" means back to Draft.
        apply.setStatus("0"); // Back to Draft
        apply.setApprovalStatus("0"); // Reset
        apply.setUpdateBy(SecurityUtils.getUsername());
        updateOpsReleaseApply(apply);
    }
}